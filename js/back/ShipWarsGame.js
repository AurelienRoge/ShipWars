...
class shipWarsGame {
    ...
    getWinner() {
        if (this.isMapFinished(this.GameMapPlayer0)) {
            this.winner = this.players[0].getID(); //Le gagnant est le joueur 1
            return this.winner;
        }
    ...
    getLoser() {
        ...
        if (this.isMapFinished(this.GameMapPlayer1)) {
            this.winner = this.players[0].getID(); //Le perdant est le joueur 1
            return this.loser;
        }

        this.loser = undefined //Sinon, il n'y a pas de gagnant
        return this.loser;
    }
    ...
    getGameMap(player){
        player === 0 ? this.GameMapPlayer0 : this.GameMapPlayer1;
        return player;
    }
    ...
    getGrid(gridOwner, hideShips){
        let gridToReturn = this.getGameMap(gridOwner);
        if(hideShips){//On cache les bateaux avant d'envoyer la carte
            gridToReturn = this.getGameMap(gridOwner);
            for(let i = 0; i < gridWithHiddenShips.length; i++){
                if(gridWithHiddenShips[i] == "B"){
                    gridWithHiddenShips[i] == "V";
                }
            }
        }
        return gridToReturn;
